...
def test_corsair_psu_get_status(mockPsuDevice):
    ...
    assert status['Input current'] == (approx(.6748, rel=1e-3), 'A')
    assert status['Input power'] == (approx(155.2, rel=1e-3), 'W')
    assert status['Total power (output)'] == (approx(140, rel=1e-3), 'W')
    assert status['Estimated efficiency'] == (approx(90.2, rel=1e-3), '%')
